...
BINDING_HEADER_BIAOGE = BINDING_HEADER_BIAOGE or "|cff00BFFFGold Group|r BiaoGe"
BINDING_NAME_ROLEOVERVIEW = "Toggle Role Overview"
...
do
    ...
    L["BiaoGe插件提醒：由于暴雪对插件UI数量的限制，本插件无法在硬核服务器使用，请你自行禁用。"] = "BiaoGe Addon Reminder: Due to Blizzard's UI limit for addons, this addon cannot be used on Hardcore servers. Please disable it manually."
    ...
    L["也有可能是你的WCL插件缺少本服务器数据包。鼠标指向一个玩家时，如果不显示WCL数据，说明就是上述问题。此问题与BiaoGe插件无关。"] = "It may also be that your WCL addon lacks data files for this server. If no WCL data shows when hovering over players, that is the cause. This issue has nothing to do with BiaoGe."
    L["保存"] = "Save"
    L["分享"] = "Share"
    L["导出"] = "Export"
    L["应用"] = "Apply"
    ...
    L['用户ID：%s'] = "User ID: %s"
    L['用户名：%s'] = "Username: %s"
    L['订阅剩余时间：%s'] = "Subscription Remaining: %s"
    L['订阅有效期：%s'] = "Subscription Valid Until: %s"
    L['天'] = "Days"
    L['流拍装备卖店总价：%s金'] = "Total vendor value of unsold items: %s Gold"
    L["搜索装备/买家/金额"] = "Search Item / Buyer / Amount"
    L["团长使用教程：输入命令/ai打开BiaoGeAI插件，点击一键安排，再点击发送。"] = "Raid Leader Guide: Type /ai to open BiaoGeAI, click One-Click Arrange, then click Send."
    L["团长使用教程：输入命令/tj打开TuanJian插件，点击一键安排，再点击发送。"] = "Raid Leader Guide: Type /tj to open TuanJian, click One-Click Arrange, then click Send."
    L["团长需使用TuanJian插件才能发送站位图，该插件可在各大插件平台免费下载。"] = "The raid leader must use the TuanJian AddOn to send position maps. It is available for free from major AddOn platforms."
    L["|cffff0000（BiaoGeLib插件被你禁用了）"] = "|cffff0000(BiaoGeLib addon is disabled)"
    ...
    L["显示副本简称"] = "Show Raid Abbreviations"
    L["开启时：角色总览列标题使用紧凑副本简称（如 SW、TOC、NAXX）。"] = "When enabled, column titles in Character Overview use compact abbreviations (e.g. SW, TOC, NAXX)."
    L["关闭时：使用完整副本名称。"] = "When disabled, full raid names are used."
    ...
    L["备注："] = "Remarks:"
    L["天"] = "d"
    L["显示角色备注"] = "Show Character Remarks"
    L["在角色名字后面，增加显示一段自定义文本。"] = "Add a section of custom text after the character name."
    L["宽度:"] = "Width:"
    ...
    L["启用该模块"] = "Enable this module"
    L["该模块已被禁用，右下角可以启用该模块"] = "This module has been disabled. You can enable it in the bottom-right corner."
    L["该模块已被禁用，右下角可以启用模块"] = "This module has been disabled. You can enable it in the bottom-right corner."
    L["该模块已禁用（重载后生效）"] = "This module has been disabled (takes effect after reloading)"
    L["该模块已启用（重载后生效）"] = "This module has been enabled (takes effect after reloading)"
    ...
    L["点击角色名字：使面板固定显示"] = "Click a character name to pin the panel."
    L["没有该角色的装备数据。"] = "No equipment data is available for this character."
    L["请打开同步软件并登录一次该角色。"] = "Open the sync application and log in to this character once."
    L["请登录一次该角色。"] = "Log in to this character once."
    L["缺少附魔！"] = "Missing enchant!"
    L["缺少宝石！"] = "Missing gem!"
    L["装等："] = "Item Level: "
    L["子账号："] = "Sub-accounts: "
    L["全部子账号"] = "All Sub-accounts"
    L["|cffff0000（角色存在重复情况，会导致同步失效和数据错误。请登录上述账号，表格设置-角色配置-删除角色。）|r"] = "|cffff0000(This character exists in more than one account, which can break synchronization and cause incorrect data. Log in to the accounts above, then go to BiaoGe Settings - Character Configuration - Delete Character.)|r"
    ...
    L["YY："] = "YY:"
    L["评价："] = "Rating:"
    L["评价仅自己可见"] = "Rating visible only to you"
    L["理由："] = "Reason:"
    ...
    L["调整装备记录通知、交易通知和减伤链进度条的位置。"] = "Adjust the positions of loot notices, trade notices, and cooldown reminder bars."
    L["快捷命令：/BGM"] = "Shortcut 'Enchant Bracer - Superior Strength' : /BGM"
    L["快捷命令：/bgmap 或 /tjmap"] = "Shortcut commands: /bgmap or /tjmap"
...
do
    ...
    L["出价设为："] = "Bid set to: "
    L["心理价格："] = "Maximum Bid: "
    L["心理价格已满，请先删除一条心理价格。"] = "The maximum bid list is full. Delete an entry first."
    L["%s的心理价格已设为：%s"] = "Maximum bid for %s set to: %s"
    L["提示：CTRL+ALT+点击表格装备，可预设心理价格。"] = "Tip: CTRL+ALT+click a table item to preset its maximum bid."
    L["提示：ALT+右键点击表格装备，可添加心理价格。"] = "Tip: ALT+right-click a table item to add a maximum bid."
    L["全部启用"] = "Enable All"
    L["全部禁用"] = "Disable All"
    L["全部清空"] = "Clear All"
    L["预设心理价格"] = "Preset Maximum Bids"
    L["启用"] = "Enable"
    L["关闭"] = "Close"
...
do -- Mail history
    ...
    -- Preset auction prices
    L["%s件"] = "%s items"
    L["10人|cff00BFFF普通"] = "10-player |cff00BFFFNormal"
    L["10人|cffff0000英雄"] = "10-player |cffff0000Heroic"
    L["25人|cff00BFFF普通"] = "25-player |cff00BFFFNormal"
    L["25人|cffff0000英雄"] = "25-player |cffff0000Heroic"
    L["ALT+%s点击表格/背包/聊天框装备时，直接按你的预设价开拍。"] = "ALT+%s-click an item in the table, bags, or chat to auction it at your preset price."
    L["ALT+点击：按预设价全部开拍"] = "ALT-click: auction all at preset prices"
    L["把当前选择的%s件装备设置为该起拍价。"] = "Set the starting price of the %s selected items."
    L["把当前选择的%s件装备设置为该起拍语。"] = "Set the auction note of the %s selected items."
    L["把序号%s-%s的装备设置为该起拍价。"] = "Set the starting price of items %s-%s."
    L["把序号%s-%s的装备设置为该起拍语。"] = "Set the auction note of items %s-%s."
    L["把一共%s件装备设置为该起拍价。"] = "Set the starting price of all %s items."
    L["把一共%s件装备设置为该起拍语。"] = "Set the auction note of all %s items."
    L["绑定类型："] = "Bind type:"
    L["不绑定"] = "Not bound"
    L["代码错误！识别失败！"] = "Invalid code. Recognition failed!"
    ...
    L["普通"] = "Normal"
    L["起拍价"] = "Starting price"
    L["起拍语"] = "Auction note"
    L["起拍语："] = "Auction note:"
    L["起拍语会显示在你拍卖一个装备的团队消息后面。"] = "The auction note is appended to the raid message when you auction an item."
    L["设置本页起拍价"] = "Set page prices"
    L["设置本页起拍价："] = "Set page prices:"
    L["设置本页起拍语"] = "Set page notes"
    L["设置本页起拍语："] = "Set page notes:"
    L["设置全部起拍价"] = "Set all prices"
    L["设置全部起拍价："] = "Set all prices:"
    L["设置全部起拍语"] = "Set all notes"
    L["设置全部起拍语："] = "Set all notes:"
    L["设置已选起拍价"] = "Set selected prices"
    L["设置已选起拍价："] = "Set selected prices:"
    L["设置已选起拍语"] = "Set selected notes"
    L["设置已选起拍语："] = "Set selected notes:"
    L["拾取绑定"] = "Binds when picked up"
    L["使用绑定"] = "Binds when used"
    L["未预设起拍价"] = "No starting price set"
    L["无职业限制"] = "No class restriction"
    L["已预设起拍价"] = "Starting price set"
    L["有职业限制"] = "Has class restriction"
    L["预设价格"] = "Preset prices"
    L["预设价格导入成功：%s，一共导入%s个价格。"] = "Preset prices imported for %s: %s prices total."
    L["预设起拍价："] = "Preset starting price:"
    L["预设起拍价：|cffffffff"] = "Preset starting price: |cffffffff"
    L["预设起拍语：|cffffffff"] = "Preset auction note: |cffffffff"
    L["职业限制："] = "Class restriction:"
    L["装备绑定"] = "Binds when equipped"
    L["装备部位："] = "Equipment slot:"
    L["装备等级："] = "Item level:"
    L["装备栏是空的"] = "The item list is empty"
    L["装备名字："] = "Item name:"
    L["装备品质："] = "Item quality:"
    L["装备属性：\n（单选）"] = "Item attribute:\n(single choice)"
    L["开拍"] = "Start"
    L["对拾取框里有预设价格的装备发起拍卖。"] = "Start auctions for loot with preset prices."
    L["%s%s|cffffffff(%s)|r |cffFFD100起拍价:%s|r"] = "%s%s|cffffffff(%s)|r |cffFFD100Starting price: %s|r"
    L["点击后以下装备将发起拍卖："] = "Click to start auctions for:"
    L["拾取框里没有预设价格的装备。"] = "No loot has a preset price."
    L["在BiaoGe底部预设价格标签里设价格。"] = "Set prices in the Preset Prices tab at the bottom of BiaoGe."
    L["成功导入%d个模版。"] = "Successfully imported %d templates."
    L["打开支出模版菜单"] = "Open the expenditure template menu"
    L["导出模版"] = "Export templates"
    L["导入"] = "Import"
    L["导入模版"] = "Import templates"
    L["导入时清空现有模板"] = "Clear existing templates when importing"
    L["复制下方文本并保存"] = "Copy and save the text below"
    L["改名"] = "Rename"
    L["管理模版"] = "Manage templates"
    L["模版"] = "Template "
    L["模版1"] = "Template 1"
    L["模版为空，无数据可导出。"] = "The template list is empty. There is no data to export."
    L["确认导入"] = "Confirm import"
    L["确认删除\"%s\"？"] = "Delete \"%s\"?"
    L["删除"] = "Delete"
    L["使用"] = "Use"
    L["使用："] = "Use: "
    L["未发现有效的模版数据或格式不正确。"] = "No valid template data was found, or the format is incorrect."
    L["新增"] = "New"
    L["粘贴之前导出的文本"] = "Paste previously exported text"
    L["正在更改\"%s\"的名字"] = "Rename \"%s\""
    L["支出"] = "Expenditure"
    L["支出模版"] = "Expenditure templates"
    L["支出项"] = "Expenditure item"
    L["合并罚款"] = "Merge fines"
    L["把相同玩家的多项罚款合并为一项，罚款内容、罚款金额和欠款都会合并一起。"] = "Merge multiple fines for the same player, including descriptions, amounts, and outstanding debt."
    L["清空罚款"] = "Clear fines"
    L["打开罚款菜单"] = "Open the fine menu"
    L["确定清空全部罚款？"] = "Clear all fines?"
end
...
do -- Batch mail
    ...
    L["特定字符:"] = "Variable:"
    L["实际效果:"] = "Result:"
    L[ [[比如：在主题里输入"工资{A}金"，在邮寄金额里输入"2000"，那么实际发送的主题是"工资2000金"。]] ] = [[For example, a subject of "Wages {A}g" and an amount of "2000" produces "Wages 2000g".]]
    L["邮寄记录"] = "Mail history"
    L["实际发送效果："] = "Preview:"
    L["邮寄人数："] = "Recipients:"
    L["邮寄金额："] = "Amount:"
...
do -- Group welcome messages
    L["进组欢迎语"] = "Group Welcome"
    L["启用进组欢迎语"] = "Enable group welcome messages"
    L["启用进组欢迎语（生效中）"] = "Enable group welcome messages (active)"
    L["启用进组欢迎语（需要创建集结号活动才生效）"] = "Enable group welcome messages (requires an active Meeting Stone listing)"
    L["当有人进组时，自动密语对方。"] = "Automatically whisper players when they join the group."
    L["当有人进组时，自动密语对方。一个输入框为一条密语。"] = "Automatically message players when they join. Each input box is sent as a separate message."
    L["点击黄色齿轮修改密语内容。"] = "Click the yellow gear to edit the message."
    L["发送频道：密语"] = "Channel: Whisper"
    L["发送频道：团队"] = "Channel: Raid"
    L["屏蔽欢迎语"] = "Hide welcome messages"
    L["在聊天框不显示你发送的欢迎语，避免刷屏。但实际上欢迎语已经发送给对方，对方能看到。"] = "Hide sent welcome messages from your chat window. Recipients will still receive them."
    L["启用该消息"] = "Enable this message"
    L["分金规则："] = "Payout rules:"
    L["拍卖规则："] = "Auction rules:"
    L["模版一"] = "Template 1"
    L["模版二"] = "Template 2"
    L["模版三"] = "Template 3"
    L["模版四"] = "Template 4"
    L["模版五"] = "Template 5"
    L["模版%s"] = "Template %s"
    L["模版改名"] = "Rename template"
    L["开组宝库时，不会发送欢迎语。"] = "Welcome messages are not sent for Vault of Archavon groups."
...
do -- Raid cooldown receiver
    L["锁定"] = "Lock"
    L["框体可还原位置"] = "Frame positions can be reset"
    L["战术"] = "Tactics"
    L["减伤链提醒"] = "Raid Cooldown Reminders"
    L["启用减伤链提醒"] = "Enable raid cooldown reminders"
    L["启用后接收TuanJian团长发送的减伤链；关闭后不显示进度条、不播放语音，也不发送团长密语提醒。"] = "Receive TuanJian cooldown assignments when enabled. Disabling stops bars, voice alerts, and leader whispers."
    L["减伤链语音提醒"] = "Cooldown voice alerts"
    L["当减伤链安排到你时，播放技能准备和交减伤技能语音。"] = "Play ready and use-cooldown voice alerts when an assignment is yours."
    L["团长密语提醒"] = "Leader whisper reminders"
    L["当你是减伤链发送者时，在倒数5秒时给相应团员发送密语提醒。"] = "When you sent the cooldown plan, whisper the assigned player at 5 seconds remaining."
    L["当你是减伤链发送者时，在施放前密语被安排的玩家。"] = "Whisper assigned players before use when you sent the cooldown plan."
    L["预览减伤链进度条"] = "Preview cooldown reminder bars"
    L["战斗中无法修改减伤链选项"] = "Cooldown reminder options cannot be changed in combat"
    L["显示谁的减伤"] = "Assignments to show"
    L["显示谁的减伤："] = "Assignments to show:"
    L["团长看全部，团员看自己"] = "Leaders: all; members: own"
    L["当我是团长/助理时，显示所有人的减伤，否则仅显示我的减伤"] = "As leader/assistant, show everyone's cooldowns; otherwise, show only mine"
    L["当我是团长/助理时显示所有人的减伤，否则仅显示我的减伤"] = "As leader/assistant, show everyone's cooldowns; otherwise, show only mine"
    L["显示所有人的减伤"] = "Show all assignments"
    L["仅显示我的减伤"] = "Show only my assignments"
    L["减伤链提前显示时间"] = "Cooldown reminder lead time"
    L["只在进入设定的最后若干秒时显示减伤链进度条。"] = "Show a cooldown bar only during the configured final seconds."
    L["提前显示时间（秒）"] = "Lead time (seconds)"
    L["减伤链进度条缩放"] = "Cooldown bar scale"
    L["调整减伤链进度条的大小。"] = "Adjust the size of cooldown reminder bars."
    L["玩家A"] = "Player A"
    L["（测试）"] = " (Test)"
    L["你"] = "You"
    L["现在"] = "Now"
    L["随时"] = "Anytime"
    L["技能准备"] = "Cooldown ready"
    ...
    L["团本报表"] = "Raid Report"
    L["< 团本报表 >"] = "< Raid Report >"
    L["团本报表说明"] = "Raid Report Instructions"
    L["团本报表 · 打工进度战报"] = "Raid Report · Work Progress"
    L["如遇任何异常或有优化建议，欢迎前往  网易DD频道: 434056 交流反馈！"] = "Feedback: Welcome to NetEase DD Channel: 434056!"
    L["资金看板"] = "Raid Report"
    L["< 资金看板 >"] = "< Raid Report >"
    L["打工看板"] = "Raid Report"
    L["周四07:00重置CD，多角色打工收益、装备消费与资金总览。"] = "Resets Thursday 07:00 CD. Multi-character income, expense & finance overview."
    L["由 BGLite_Plus 波比兔 出品"] = "Created by BGLite_Plus Bobitu"
    L["(周四07:00重置CD | 多角色打工与资金总览 | 波比兔出品)"] = "(Resets Thu 07:00 | Multi-Char Finance Overview | by Bobitu)"
    L["本周 CD"] = "This Week"
    L["本周 CD (当前)"] = "This Week (Current)"
    L["上周 CD"] = "Last Week"
    L["近 30 天"] = "Last 30 Days"
    L["全部历史"] = "All Time"
    L["全部角色"] = "All Characters"
    L["+ 记一笔 / 补贴"] = "+ Add Entry / Subsidy"
    L["战报分享"] = "Share Report"
    L["累计工资收入"] = "Total Wage Income"
    L["打工吸金榜首 (劳模)"] = "Top Earner (MVP)"
    L["单车效率 / 额外补贴"] = "Avg per Run / Subsidies"
    L["打工与收支明细"] = "Income & Expense Log"
    L["打工收益明细流水"] = "Wage Income Log"
    L["共出勤 %d 车 %s"] = "%d Runs %s"
    L["贡献 %s (占比 %d%%, %d车)"] = "Earned %s (%d%%, %d runs)"
    L["暂无数据"] = "No Data"
    L["打本保存表格后自动入账"] = "Auto-recorded when saving raid ledger"
    L["净到手: %s (额外补贴: %s)"] = "Net: %s (Subsidies: %s)"
    L["当前周期内暂无打工收益记录"] = "No wage records found in this cycle"
    L["补录打工收益 / 额外补贴"] = "Log Wage / Subsidy"
    L["角色名称:"] = "Character Name:"
    L["副本名称:"] = "Raid Name:"
    L["分得工资:"] = "Wage Received:"
    L["额外补贴:"] = "Extra Subsidy:"
    L["打工备注:"] = "Notes:"
    L["TN补贴/野团全通"] = "Tank/Heal Subsidy / Pug Run"
    L["保存入账"] = "Save Entry"
    L["工资和补贴不能同时为0！"] = "Wage and subsidy cannot both be 0!"
    L["已成功补录打工收益: %s %d G (补贴 %d G)"] = "Logged: %s %d G (Subsidy %d G)"
    L["当前筛选周期内暂无打工战报可供分享。"] = "No report available for the selected cycle."
    L["[BGLite_Plus 波比兔] 周四打工周报"] = "[BGLite_Plus Bobitu] Thursday Raid Report"
    L["上周出勤 %d 车，累计赚取工资 %s !"] = "Last week: %d runs, earned %s !"
    L["查看看板"] = "View Dashboard"
    L["我知道了"] = "Got it"
    L["累计总支出"] = "Total Spend"
    L["支出"] = "Spend"
    L["消费最多: %s (%s, %d%%)"] = "Top Spender: %s (%s, %d%%)"
    L["消费榜首: %s (-%s, 占比 %d%%)"] = "Top Spender: %s (-%s, %d%%)"
    L["暂无装备自购与罚款支出"] = "No spend or penalty records"
    L["单车效率 / 实际净到手"] = "Efficiency / Net Profit"
    L["累计总支出明细"] = "Total Spend Breakdown"
    L["总计支出: -%s"] = "Total Spend: -%s"
    L["装备自购: -%s"] = "Gear Purchased: -%s"
    L["罚款支出: -%s"] = "Penalties: -%s"
    L["各角色消费排行:"] = "Character Spending Rank:"
end
