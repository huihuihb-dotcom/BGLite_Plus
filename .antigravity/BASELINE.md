...
* **预设价格 (AuctionPreset) 模块落成与团长拍卖发起深度联动 (2026-09-04)**:
  ...
  - **副本重置时间直观格式化（改以天和小时展示） (2026-09-04)**:
    - **背景痛点**: 原版调用 `BG.SecondsToTime` 仅以小时为最大单位（如 5 天 14 小时被粗暴显示为 `134h` / `134小时`），缺乏天数维度且难以心算剩余天数；
    - **实施改动**: 重构 [RoleOverview_core.lua](file:///e:/World%20of%20Warcraft/_classic_titan_/Interface/AddOns/BGLite_Plus/Core/RoleOverview_core.lua) 中的 `SafeFormatResetTime` 算法，凡大于等于 1 天的时间严格格式化为 `X天Y小时`（整天则为 `X天`），不足 1 天但大于等于 1 小时格式化为 `X小时Y分钟`，不足 1 小时格式化为 `Z分钟`；
    - **呈现效果**: 总览面板标题处清晰呈现为 `（团本重置时间：4天18小时, 小团本1天8小时）`，一目了然。

## 9. 下一步计划 / 待办事项
- 观察时光服实战中在野外进组时新团队 YY 号和招募通告的即时刷新表现。
- 观察时光服实战中打完太阳井进祖阿曼（或反向进本）时自动清空拦截日志与账目保留完整性。
