...
## 56. 预设装备单件自动拍卖控制（保留人工处理）与套装批量修改落地 (2026-09-09)
* **需求背景与业务场景**:
  - 团长带团实战中，常规装备拾取后可自动开拍；但部分极品散件、高价值饰品、武器或特殊掉落，团长希望**不自动开拍**，保留在背包并在打完全团后**单独人工拍卖/协商处理**；
  - 团本中掉落的套装兑换物（Token/印记/奖章）数量较多，团长希望能够**一键批量修改所有套装是否自动拍卖**；
  - 其他装备默认跟随全局的【拾取后自动拍卖】总开关（开启则自动，关闭则不自动）。
* **核心架构与技术实现**:
  1. **数据存储与默认跟随模型 (`BiaoGe.auctionPreset[FB].autoAuction`)**:
     - 每一件装备对应 `autoAuction[itemID]`：
       - `nil`: 默认跟随全局开关 `BiaoGe.options.autoAuctionOnLoot`（总开关开启则自动开拍）；
       - `0`: 显式配置为【不自动拍卖】（保留人工处理）；
       - `1`: 显式配置为【开启自动拍卖】。
  2. **预设列表行内复选框交互 (`row.autoCheck`)**:
     - 在【预设价格】列表表头扩展【自动】列（宽度 45px，居中）；
     - 每一行嵌入暴雪原生 CheckButton，状态严格对齐 `(data.autoAuction == nil) or (data.autoAuction == 1)`；
     - 玩家取消勾选时，立即写入 `autoAuction[itemID] = 0` 并持久化；
     - 悬停 Tooltip 清晰提示当前状态与业务说明；
     - 行末红叉清空与副本【全部清空】同步联动清除单项配置并重置回默认跟随。
  3. **【自动拍卖 ▾】批量设置工具栏 (`batchSetBtn` 与下拉菜单)**:
     - 在顶栏【批量底价】右侧挂载【自动拍卖 ▾】实体按钮；
     - 菜单精简实用，拒绝多余冗余项：
       - `全部开启自动拍卖 (橙装保持人工)`: 将当前副本所有装备设为自动拍卖（橙装自动跳过保持人工保护）；
       - `全部关闭自动拍卖 (全部人工)`: 将当前副本所有装备设为不自动拍卖；
       - `套装：设为不自动 (保留人工)`: 满足“套装选择不自动，打完统一人工拍卖”的单一核心诉求，一键关闭所有套装代币；
       - `全部恢复默认跟随总开关`: 清空单项特殊配置，全量恢复跟随总开关；
     - 点击后批量更新并实时刷新列表各行复选框与底部统计，并在聊天框发送清晰通报。
  4. **摸尸体/进包待拍队列精细化拦截 (`AutoAuctionOnLoot.IsItemAutoEnabled`)**:
     - 在 `QueueItemForAuction` 入口处统一调用 `IsItemAutoEnabled(itemID, FB)`；
     - 若当前装备配置为不自动（`autoAuction == 0`），安全跳过入队，并在聊天框输出：`[BGLite 自动拍卖] 装备：[链接] 已配置为【不自动拍卖】（保留人工处理），已跳过。`；
     - 该装备稳妥留存背包，杜绝任何误拍。
  5. **底部动态统计与提示自愈**:
     - 实时统计当前副本已被配置为【不自动拍卖】的装备数量，底部动态呈现：`提示：已设置 X 件装备【不自动拍卖】(保留最后人工处理)。取消勾选【自动】即可设置。`。
  6. **深度排查与三大潜在隐患全量消除 (2026-09-09)**:
     - **隐患 A（物品分配者 0 索引异常）**: 怀旧服底层 `GetLootMethod()` 在当前玩家为队长拾取分配者时返回 `0`，此前由于 `UnitIsUnit("raid0", ...)` 导致非团长的分配者被误判拦截；已修复补全 `raidMaster == 0 or partyMaster == 0`，分配者 100% 正常开拍；
     - **隐患 B（同 Boss 掉落多件同名装备误杀）**: 此前 45 秒去重池仅通过 `itemID_link` 查重，导致同一个 Boss 掉落 2 个相同套装代币时第 2 件被直接丢弃；已增加 `slotKey` 槽位隔离，同场战斗多件同名掉落均能一件不漏全量开拍；
     - **隐患 C（加载顺序与类型失配自愈）**: 补齐 `idNum` 与 `idStr` 双类型索引兼容，并为 `AuctionPreset.lua` 内置套装识别自愈兜底，杜绝 TOC 文件依赖倒挂引起的识别失效。
  7. **橙装出厂默认人工保护体系 (Legendary Protection) (2026-09-09)**:
     - 传奇品质装备（`quality == 5`，如风剑左右脸、橙锤之眼、蛋刀、橙弓、瓦兰奈尔/影锋碎片等）价值极高且极具争议，出厂**强制默认人工处理（不自动开拍）**；
     - 列表渲染时橙装行默认不勾选，悬停提示专属橙装保护；
     - 批量开启自动拍卖时自动豁免橙装，除非团长主动单独勾选，否则 100% 绝不自动开拍，从物理机制上彻底终结高价贵重资产误拍事故。
   8. **【套装：设为不自动】批量修改误判与误杀全量装备 Bug 彻底根治 (2026-09-09)**:
      - **病根诊断**:
        1. **分支逻辑穿透**: 此前在 BatchSetAutoAuction 循环中，仅计算了 isToken 变量，但在执行 autoDB[item.itemID] = 0 时未对非套装装备做 if isToken 隔离拦截，导致循环无差别将整张表所有装备全部写入了 0；
        2. **判定函数混淆**: 原 ns.IsSetTokenOrRaidItem 设计初衷是“团本掉落与套装”防误杀白名单（内部将只要出现在金团表格里的所有团本装备均判为 true，且含头盔/胸甲等泛词），不能用于纯套装 Token 的筛选。
      - **解决方案与权威数据源支持**:
        1. **原生权威数据源支撑 (BG.Loot[FB].ExchangeItems)**: 深度接入上游 BGLite 原生针对每个副本维护的套装兑换物数据库 BG.Loot[FB].ExchangeItems，凡在该表中存在映射的物品 100% 权威判定为套装代币；
        2. **严格排除穿戴散件**: 补充魔兽专属前缀（失落的、战败的、征服者的、保卫者的、胜利者的、圣洁徽记、北伐奖章等）并校验 equipLoc，严格排除可穿戴装备与普通饰品；
        3. **严格分支隔离**: filterTokenOnly 模式下仅处理 isToken == true 的物品，非套装装备 100% 绝对跳过、绝不修改任何属性，界面复选框仅精准取消套装代币的勾选，普通装备稳稳保持默认自动。

## 9. 角色总览：显示/隐藏角色选项与管理系统 (2026-09-09)
* **背景与需求**:
  - 玩家账号中存在仓库号、练级废弃号或跨账号同步的不打本角色，希望能在角色总览（团本CD、货币统计）中按需隐藏指定角色，同时保证默认新角色 100% 正常显示。
* **架构设计与核心机制**:
  1. **反向存储设计 (默认全显原则)**:
     - 存储于 `BiaoGe.roleOverviewHideRoles[realmID][player] = true`；
     - 仅记录被玩家明确主动隐藏的角色；任何未记录的角色（包括新创建或新读出的角色）状态为 `nil`，天然保证出厂与新增时**默认 100% 全部显示**。
  2. **数据层深度过滤 (`RoleOverview_core.lua`)**:
     - 全局提供 `BG.IsRoleOverviewRoleHidden(realmID, player)`；
     - 在 `GetRaidCDdb`（团本CD）与 `GetMoneydb`（货币与资源）角色注入阶段统一拦截被隐藏角色，使团本CD、货币统计列表、新版紧凑UI、经典上下/左右布局均彻底排除该角色，且底部的货币合计（Sum）同步精准排除。
  3. **设置面板双重入口与自定义队列 (`RoleOverviewOptions.lua`)**:
     - **基础开关第 7 行**: 配备【显示/隐藏角色：】`[ 管理角色显示 ]` 按钮，并在右侧实时展示彩色状态：`全部显示（未隐藏任何角色）`（绿字）或 `已隐藏 X 个角色`（红字）；
     - **主体自定义队列 (对齐现有队列规范)**: 在货币队列后追加【角色显示自定义队列】，配备 `[ 全部显示 ]`、`[ 仅当前角色 ]`、`[ 反选 ]` 快捷操作按钮，三列平铺展示全角色卡片（含天赋图标、职业染色、装等/等级、跨服服务器标注）；
     - **独立弹窗 (`BGRoleOverviewFilterFrame`)**: 具备拖拽、ESC关闭、实时搜索框、双列卡片、快捷按钮与底部统计，边看总览边调整更加便捷。
  4. **总览面板右键快捷交互增强**:
     - 在角色总览界面（按中键固定后）右键点击角色名字，自动弹出二次确认弹窗直接隐藏此角色，并在聊天框输出友好还原指引提示。
  5. **BiaoGe 完整源码同步**:
     - 在 `c:\Users\huihu\Desktop\BiaoGe\BiaoGe` 中同步完成底层过滤与设置界面接入，保证多环境代码资产一致。

## 10. 团队工具 (RaidTool)：进组自动通知 Bug 根治与小队误发隔离 (2026-09-09)
* **用户反馈问题**:
  1. 无论是否勾选【仅团长/助理】，新进人员都会被发送进组信息；
  2. 玩家进入 5 人小队或者组 5 人队时，也会频繁误发进组密语与小队发言。
* **病根诊断与核心机理**:
  1. **名册状态机雪崩 (Roster Tracking Collapsing)**:
     - 原逻辑在 `GROUP_ROSTER_UPDATE` 事件最外层直接执行 `if db.notifyOnlyLeader and not IsLeaderOrAssistant() then return end`；
     - 当玩家单人退队或被踢出队伍时，由于已处于非 Leader 状态，事件在最开头直接被拦截，导致 `isFirstRosterScan = true` 和名册清空逻辑 `wipe(knownRosterMembers)` **从未被执行**；
     - 玩家下一次进入任意新小队或新团队时，`isFirstRosterScan` 仍然处于旧状态 `false`，导致新队伍里的所有成员被误杀判为“新进成员”，瞬间引发全队全员密集密语与公屏发言。
  2. **5 人小队长身份越权识别 (GroupLeader Overreach)**:
     - 原 `IsLeaderOrAssistant()` 使用了 `UnitIsGroupLeader("player")`；在普通 5 人小队中，只要玩家是队长，该函数始终返回 `true`；
     - 导致无论玩家勾选还是取消勾选【仅团长/助理】，只要身为小队长，就会认定“具备发送权限”，产生“勾不勾选都不影响发信息”的现象。
  3. **缺乏团队与小队的场景隔离**:
     - 团队工具（RaidTool）设计的本意是服务于团本金团活动，而在普通日常 5 人小队中发送 YY 进组通知会产生极大的尴尬与打扰；原代码甚至在小队中默认向 `PARTY` 频道刷屏。
  4. **系统消息 `"你加入了队伍"` 过滤漏洞**:
     - `CHAT_MSG_SYSTEM` 正则匹配未能排除 `"你"`，存在向自身异常发送密语的隐患。
* **解决方案与重构**:
  1. **状态机与权限判断彻底解耦**:
     - `GROUP_ROSTER_UPDATE` 无条件执行名册维护与退队重置；只要处于单人或不在队伍状态，立即干净重置名册与首扫状态；
     - 进组初次扫描一律静默建档（绝不发通知）；仅对后续真正进组的新人进行通知；
     - 权限与场景校验仅在真正派发消息前执行，绝不污染名册生命周期。
  2. **新增【仅团队(Raid)生效】（`notifyOnlyRaid`，默认开启 true）**:
     - 在【新进成员自动通知】标题下方，与【仅团长/助理】构成对称双列控制项；
     - 默认开启状态下，只要当前不是团队（Raid，10人/25人/40人），100% 物理拦截所有小队误发；
     - **精心润色悬停说明 (Tooltip)**：
       - **勾选 (默认)**：仅在团队活动中生效，5人小队或地下城中绝不发送，杜绝打日常5人本时的误发；
       - **反选 (取消勾选)**：允许在5人小队中生效，当自己为小队长组人时正常发送进组密语和小队频道欢迎公告；若为普通队员则依然受【仅团长/助理】保护不乱发。
  3. **判定体系重构 (`CanSendNewMemberNotification`)**:
     - 在团队中：强制要求是团长或团队助理(A)（若勾选仅限团长/助理）；
     - 在小队中：仅当用户手动关闭【仅团队生效】且满足小队长身份时才允许发送；
     - 异步延时 1 秒后的闭包中再次执行双重权限与在线校验。
  4. **历史过期迁移代码清理**:
