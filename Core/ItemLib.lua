...
do
                    ...
                    if itemID then
                        BG.OnItemLoad(itemID):ContinueOnItemLoad(function()
                            ...
                            if not name or not level then
                                local _id, _type, _subType, _EquipLoc, _Texture, _classID, _subclassID = GetItemInfoInstant(itemID)
                                EquipLoc = EquipLoc or _EquipLoc
                                Texture = Texture or _Texture
                                typeID = typeID or _classID
                                subclassID = subclassID or _subclassID
                                level = level or 1
    ...
    function CreateAllItemInfoCache()
        ...
        -- 历遍同阶段的多个团本
        for _, FB in pairs(BG.phaseFBtable[FB] or { FB }) do
            FBs[FB] = true
            if BG.Loot and BG.Loot[FB] then
                -- 团本
                for _, hard in ipairs(BG.difficultyTable[FB] or {}) do
                ...
                -- 牌子装备
                if BG.Loot[FB].Currency then
                    for itemID, v in pairs(BG.Loot[FB].Currency) do
                        InsertToAllItem(itemID)
                    end
                end
                -- 赛季服货币/牌子
                if BG.Loot[FB].Sod_Currency then
                    for i, v in pairs(BG.Loot[FB].Sod_Currency) do
                        for itemID, currency in pairs(v) do
                            InsertToAllItem(itemID)
                        end
                    end
                end
                -- 声望装备
                if BG.Loot[FB].Faction then
                    for k, v in pairs(BG.Loot[FB].Faction) do
                        for i, itemID in ipairs(v) do
                            InsertToAllItem(itemID)
                        end
                    end
                end
                -- 专业制造
                if BG.Loot[FB].Profession then
                ...
                -- 世界掉落
                if BG.Loot[FB].World then
                    for i, itemID in ipairs(BG.Loot[FB].World) do
                        InsertToAllItem(itemID)
                    end
                end
                -- 世界BOSS
                if BG.Loot[FB].WorldBoss then
                ...
                -- 节日
                if BG.Loot[FB].Holiday then
                    for _, holiday in pairs(BG.Loot[FB].Holiday) do
                        if type(holiday) == "table" and holiday.items then
...
-- 第二步：找出符合条件的装备
do
    local function IsYesItem(itemID, FB)
        FB = FB or BG.FB1
        ...
        local invType = BiaoGe.ItemLib and BiaoGe.ItemLib.ItemLibInvType
        if type(invType) == "string" then
            invType = { invType }
            BiaoGe.ItemLib.ItemLibInvType = invType
        elseif type(invType) ~= "table" then
            invType = { "INVTYPE_HEAD" }
            BiaoGe.ItemLib.ItemLibInvType = invType
        end

        for _, _EquipLoc in pairs(invType) do
        ...
        if BiaoGe.ItemLib and BiaoGe.ItemLib.iLevel and BiaoGe.ItemLib.iLevel[FB] and tonumber(BiaoGe.ItemLib.iLevel[FB]) then
            if (info[FB][itemID].level or 0) < tonumber(BiaoGe.ItemLib.iLevel[FB]) then
                return false
            end
        end
        ...
        if BG.FilterAll then
            local ok, filterOut = pcall(BG.FilterAll, itemID, typeID, EquipLoc, subclassID, tooltipText)
            if ok and filterOut == true then
                return false
            end
        end
        return true
    ...
    local function InsertItemInfo(FB, itemID, _type, hard, ii, other)
        if not IsYesItem(itemID, FB) then return end
    ...
    function CheckItemInfo()
        ...
        local fitlerGet = (BiaoGe and BiaoGe.ItemLib and BiaoGe.ItemLib.fitlerGet) or {}
        local phaseFBs = (BG.phaseFBtable and BG.phaseFBtable[FB]) or { FB }

        for _, FB in pairs(phaseFBs) do
            if BG.Loot and BG.Loot[FB] then
                -- 团本
                local diffs = (BG.difficultyTable and BG.difficultyTable[FB]) or {}
                for _, hard in ipairs(diffs) do
                    local trueRaidDifficulty = true
                    if BG.onlyOneHard then
                        if fitlerGet.raid then
                            trueRaidDifficulty = false
                        end
                    else
                        if fitlerGet.raidmyth and strfind(hard, "M") then
                            trueRaidDifficulty = false
                        elseif fitlerGet.raidhero and strfind(hard, "H") then
                            trueRaidDifficulty = false
                        elseif fitlerGet.raidnormal and strfind(hard, "N") then
                            trueRaidDifficulty = false
                        elseif fitlerGet.raid25 and strfind(hard, "25") then
                            trueRaidDifficulty = false
                        elseif fitlerGet.raid10 and strfind(hard, "10") then
                            trueRaidDifficulty = false
                        end
                    end
                    ...
                    if trueRaidDifficulty then
                        if BG.Loot[FB][hard] then
                            ...
                            while BG.Loot[FB][hard]["boss" .. ii] do
                                if not (FB == "TOC" and ii == 7 and hard:find("H")) then
                                    for i, itemID in ipairs(BG.Loot[FB][hard]["boss" .. ii] or {}) do
                            ...
                            if BG.Loot[FB][hard].Quest then
                                for name, _ in pairs(BG.Loot[FB][hard].Quest) do
                                    for _, itemID in pairs(BG.Loot[FB][hard].Quest[name] or {}) do
                ...
                -- 5人本
                if not fitlerGet.fb5 and BG.Loot[FB].Team then
                    for FB_5 in pairs(BG.Loot[FB].Team) do
                        for BossName, _ in pairs(BG.Loot[FB].Team[FB_5] or {}) do
                            for _, itemID in pairs(BG.Loot[FB].Team[FB_5][BossName] or {}) do
                ...
                -- 牌子装备
                if not fitlerGet.currency and BG.Loot[FB].Currency then
                    for itemID, v in pairs(BG.Loot[FB].Currency) do
                        InsertItemInfo(FB, itemID, "currency", hard, ii, v)
                    end
                end
                -- 赛季服货币/牌子
                if not fitlerGet.currency and BG.Loot[FB].Sod_Currency then
                    for i, v in pairs(BG.Loot[FB].Sod_Currency) do
                ...
                -- 声望装备
                if not fitlerGet.faction and BG.Loot[FB].Faction then
                    for k, v in pairs(BG.Loot[FB].Faction) do
                        if type(v) == "table" then
                            for i, itemID in ipairs(v) do
                ...
                -- 专业制造
                if not fitlerGet.profession and BG.Loot[FB].Profession then
                    for k, v in pairs(BG.Loot[FB].Profession) do
                        if type(v) == "table" then
                            for i, itemID in ipairs(v) do
                ...
                -- 世界掉落
                if not fitlerGet.world and BG.Loot[FB].World then
                    for i, itemID in ipairs(BG.Loot[FB].World) do
                        InsertItemInfo(FB, itemID, "world", hard, ii, k)
                    end
                end
                -- 世界BOSS
                if not fitlerGet.worldboss and BG.Loot[FB].WorldBoss then
                    for k, v in pairs(BG.Loot[FB].WorldBoss) do
                        if type(v) == "table" then
                            for i, itemID in ipairs(v) do
                ...
                -- PVP
                if not fitlerGet.pvp and BG.Loot[FB].PVP then
                    for k, v in pairs(BG.Loot[FB].PVP) do
                        if type(v) == "table" then
                            for i, itemID in ipairs(v) do
                ...
                -- PVP货币
                if not fitlerGet.pvp and BG.Loot[FB].PVP_currency then
                    for itemID, v in pairs(BG.Loot[FB].PVP_currency) do
                        InsertItemInfo(FB, itemID, "pvp_currency", hard, ii, v)
                    end
                end
                -- 商店
                if not fitlerGet.shop and BG.Loot[FB].Shop then
                    for _, v in pairs(BG.Loot[FB].Shop) do
                        if type(v) == "table" and v.id then
                ...
                -- 节日
                if not fitlerGet.holiday and BG.Loot[FB].Holiday then
                    for _, holiday in pairs(BG.Loot[FB].Holiday) do
                        if type(holiday) == "table" and holiday.items then
                            for _, itemID in pairs(holiday.items) do
...
local function SetItemLib()
    if not mainFrame.bg then return end
    mainFrame.bg:Show()
    mainFrame.scroll:Show()
    mainFrame.child:Show()

    local baseLevel = mainFrame:GetFrameLevel() or 10
    mainFrame.bg:SetFrameLevel(baseLevel + 5)
    mainFrame.scroll:SetFrameLevel(baseLevel + 10)
    mainFrame.child:SetFrameLevel(baseLevel + 15)
        ...
        for i, v in ipairs(titleTbl) do
            ...
            f:SetScript("OnMouseDown", function(self, button)
                if button == "LeftButton" and not IsShiftKeyDown() and not IsControlKeyDown() and not IsAltKeyDown() then
                    if mainFrame.buttons[ii] and mainFrame.buttons[ii].item and mainFrame.buttons[ii].item.hope and mainFrame.buttons[ii].item.hope:IsVisible() then return end
                    ...
                    BG.PlaySound(1)
                elseif BG.IsSetBestPriceKeyDown(button == "RightButton") then
...
local function UpdateTiptext()
    ...
    local P = (BG.GetFBinfo and BG.GetFBinfo(FB, "phase")) or (FB or "")
    local B = ""
    local invTypes = BiaoGe.ItemLib and BiaoGe.ItemLib.ItemLibInvType
    local invFirst = type(invTypes) == "table" and invTypes[1] or (type(invTypes) == "string" and invTypes or "INVTYPE_HEAD")
    for i, v in ipairs(BG.invtypetable or {}) do
        if v.key and v.key[1] == invFirst then
            B = (v.name)
        end
    end
...
local function StartUpdate()
    ...
    CheckSameItem() -- 合并重复装备与途径
    Sort()          -- 排序
    SetItemLib()    -- 生成列表
    UpdateTiptext()
end
function BG.UpdateItemLib()
    mainFrame = BG.ItemLibMainFrame or mainFrame
    if rawget(mainFrame, "IsVisible") then mainFrame.IsVisible = nil end
    if rawget(mainFrame, "IsShown") then mainFrame.IsShown = nil end
    if not mainFrame:IsVisible() then
        return
    end
    BG.itemLibNeedUpdate = false
    local FB = BG.FB1
    if not (info[FB] and info[FB]._isDone) then -- 如果还没缓存完成该副本，则先缓存
        dbOK = false
        local t = CreateLoadingText()
        CreateAllItemInfoCache()
        BG.OnUpdateTime(function(self, elapsed)
            if dbOK then
                if info[FB] then info[FB]._isDone = true end
                self:SetScript("OnUpdate", nil)
                self:Hide()
                t:Hide()
                StartUpdate()
...
BG.Init2(function()
    mainFrame = mainFrame or BG.ItemLibMainFrame
