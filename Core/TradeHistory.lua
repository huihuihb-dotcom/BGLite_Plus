...
local function RoadTrade()
            ...
            elseif event == "UI_INFO_MESSAGE" then
                ...
                if text == ERR_TRADE_COMPLETE then
                    ...
                    After(.5, function()
                        ...
                        if BG.TradeHistoryMainFrame and BG.TradeHistoryMainFrame.frame and BG.TradeHistoryMainFrame.frame:IsVisible() then
                            BG.UpdateTradeHistoryScrollFrame()
                        end
                    end)

                    if BiaoGe.options and BiaoGe.options.tradeSuccessSound == 1 then
                        BG.PlaySound("tradeSuccess")
                    end
                    if BiaoGe.options and BiaoGe.options.tradeMSG == 1 and BiaoGe.options.tradeMSG_success == 1 then
            ...
            elseif event == "TRADE_CLOSED" then
                After(0, function()
                    if not success and target then
                        if BiaoGe.options and BiaoGe.options.tradeFalseSound == 1 then
                            BG.PlaySound("tradeFalse")
                        end
                        if BiaoGe.options and BiaoGe.options.tradeMSG == 1 and BiaoGe.options.tradeMSG_false == 1 then
                            local channel = BiaoGe.options["tradeMSG_channel"]
                            if channel ~= "WHISPER" then
                                if IsInRaid(1) then
                                    channel = "RAID"
    ...
    do
        ...
        if BiaoGe.tradeHistory and BiaoGe.tradeHistory.isChooseRealm == 1 then choose = { realmID = realmID, } end
        ...
        do
            ...
            bar.scrollStep = 5
            if BG.CreateSrollBarBackdrop then BG.CreateSrollBarBackdrop(bar) end
            bar:SetScript("OnValueChanged", function(self)
                self:SetScript("OnUpdate", function(self, t)
                    UpdateButtons()
                    UpdateScrollButtonState()
                    if mainFrame.infoFrame then mainFrame.infoFrame:Hide() end
                    ...
                    f:SetScript("OnMouseUp", function(self, button)
                        ...
                        local menu = {
                            ...
                            },
                            {
                                text = CANCEL,
                                notCheckable = true,
                                func = LibBG and LibBG.CloseDropDownMenus or nil,
                            }
                        }
                        if LibBG and LibBG.EasyMenu then
                            LibBG:EasyMenu(menu, BG.dropDown or dropDown, "cursor", 0, 0, "MENU", 2)
                        end
                        if BG.PlaySound then BG.PlaySound(1) end
                    end)
                end
        ...
        do
            local function DeleteTradeData()
                ...
                for realmID in pairs(BiaoGe.tradeHistory or {}) do
                    if type(BiaoGe.tradeHistory[realmID]) == "table" then
                        for player in pairs(BiaoGe.tradeHistory[realmID]) do
                            for i = #BiaoGe.tradeHistory[realmID][player].info, 1, -1 do
                                ...
                                if BiaoGe.tradeHistory.saveDuration and BiaoGe.tradeHistory.saveDuration > 0 and time - (v.time or 0) > 86400 * BiaoGe.tradeHistory.saveDuration then
                                    tremove(BiaoGe.tradeHistory[realmID][player].info, i)
                                end
                            end
                        end
            ...
            text:SetTextColor(RGB(BG.y2 or "FFFF00"))
            text:SetText(L["保存时长："])
            ...
            if BG.dropDownToggle then BG.dropDownToggle(dropDown) end
            LibBG:UIDropDownMenu_Initialize(dropDown, function(self, level)
                if not BG.saveDays then return end
                for i, v in ipairs(BG.saveDays) do
        ...
        do
            ...
            local realmLabel = (BiaoGe.realmName and BiaoGe.realmName[realmID]) or realmID
            if BiaoGe.tradeHistory and BiaoGe.tradeHistory.isChooseRealm == 1 then
                LibBG:UIDropDownMenu_SetText(dropDown, (BG.STC_y2 and BG.STC_y2(realmLabel .. " - " .. L["全部角色"])) or (realmLabel .. " - " .. L["全部角色"]))
            else
                local rText = (BG.STC_y2 and BG.STC_y2(realmLabel .. " - ")) or (realmLabel .. " - ")
                local pText = (SetClassCFF and SetClassCFF(player, "player")) or player
                LibBG:UIDropDownMenu_SetText(dropDown, rText .. pText)
            end

            LibBG:UIDropDownMenu_Initialize(dropDown, function(self, level)
                for _realmID, v in pairs(BiaoGe.tradeHistory or {}) do
                    if type(v) == "table" then
                        local rName = (BiaoGe.realmName and BiaoGe.realmName[_realmID]) or _realmID
                        if next(BiaoGe.tradeHistory[_realmID]) then
                            local info = LibBG:UIDropDownMenu_CreateInfo()
                            info.text = (BG.STC_y2 and BG.STC_y2(rName)) or rName
                            ...
                            info.func = function()
                                ...
                                LibBG:UIDropDownMenu_SetText(dropDown, (BG.STC_y2 and BG.STC_y2(rName .. " - " .. L["全部角色"])) or (rName .. " - " .. L["全部角色"]))
                                UpdateScrollFrame()
                                UpdateButtons()
                            end
                            LibBG:UIDropDownMenu_AddButton(info)
                        ...
                        for _, v in pairs(BiaoGe.tradeHistory[_realmID]) do
                            local info = LibBG:UIDropDownMenu_CreateInfo()
                            local classColor = v.class and select(4, GetClassColor(v.class)) or "ffffff"
                            local playerName = "|c" .. classColor .. v.name
                            info.text = "  " .. playerName .. (v.level and " (" .. v.level .. ")" or "")
                            info.arg1 = v.realmID .. "-" .. v.name
                            info.arg2 = "|cffFFD100" .. rName .. "-|r|c" .. classColor .. v.name .. "|r"
                            ...
                            info.func = function()
                                ...
                                local rPrefix = (BG.STC_y2 and BG.STC_y2(rName .. " - ")) or (rName .. " - ")
                                LibBG:UIDropDownMenu_SetText(dropDown, rPrefix .. playerName)
                                UpdateScrollFrame()
                                UpdateButtons()
                            end
                            LibBG:UIDropDownMenu_AddButton(info)
            ...
            StaticPopupDialogs["BiaoGe_DeleteTradeHistoryPlayer"] = {
                ...
                OnAccept = function()
                    ...
                    if _realmID and _player and BiaoGe.tradeHistory[_realmID] then
                        BiaoGe.tradeHistory[_realmID][_player] = nil
                        if _realmID == realmID and _player == player then
                            EnsureCurrentCharacter()
                        end
                        BG.UpdateTradeHistoryScrollFrame()
                        if SendSystemMessage and dropDown.colorPlayer then
        ...
        do
            ...
            if edit.Instructions then edit.Instructions:SetText(L["搜索交易对象"]) end
            mainFrame.serachEdit = edit
            if BG.SetEditBaseClass then BG.SetEditBaseClass(edit) end
            edit:HookScript("OnTextChanged", function(self)
                BG.UpdateTradeHistoryScrollFrame()
            end)
            edit:HookScript("OnEditFocusGained", function(self)
                BG.lastfocus = self
                if BG.GetVerNum and BG.ver and BG.GetVerNum(BG.ver) >= 12820 then
                    if BG.SetListmaijia then BG.SetListmaijia(self, true, nil, nil, true) end
                end
            end)
        ...
        do
            for i, v in ipairs(titleTbl) do
                ...
                bt:SetNormalFontObject(BG["FontWhite" .. FONTSIZE] or "GameFontHighlight")
        ...
        function GetDB()
            ...
            if choose.player then
                ...
                for player, v in pairs(BiaoGe.tradeHistory[realmID] or {}) do
                    tinsert(playerTbl, player)
                end
            end

            for _, player in pairs(playerTbl) do
                for i, v in ipairs((BiaoGe.tradeHistory[realmID] and BiaoGe.tradeHistory[realmID][player] and BiaoGe.tradeHistory[realmID][player].info) or {}) do
        ...
        function UpdateScrollButtonState()
            ...
            if scrollUpButton then scrollUpButton:Enable() end
            if scrollDownButton then scrollDownButton:Enable() end
        ...
        function GetButtonInfo(num)
            ...
            local item
            for i = 1, 6 do
                if v.playeritems and v.playeritems[i] then
                    count = count + 1
                    local countText = ""
            ...
            local playerMoneyText = ""
            local money = v.playermoney or 0
            if money > 0 then
            ...
            local item
            for i = 1, 6 do
                if v.targetitems and v.targetitems[i] then
                    count = count + 1
                    local countText = ""
            ...
            local targetMoneyText = ""
            local money = v.targetmoney or 0
            if money > 0 then
            ...
            return {
                ...
                date("%m-%d\n%H:%M", v.time), -- 时间
                v.zone or "", -- 地点
                "|c" .. (v.playerClass and select(4, GetClassColor(v.playerClass)) or "ffffff") .. (v.playerName or "") .. "|r", -- 角色
                "|c" .. (v.targetClass and select(4, GetClassColor(v.targetClass)) or "ffffff") .. (v.targetName or "") .. "|r", -- 交易对象
                playerItemText .. playerSplit .. playerMoneyText, -- 交出
                targetItemText .. targetSplit .. targetMoneyText, -- 收到
                GetMoneyString(floor((v.beforeMoney or 0) / 1e4) * 10000, true), -- 交易前财产
        ...
        do
            ...
            do
                ...
                frame:HookScript("OnEnter", function(self)
                    ...
                    if num and buttons[num] and buttons[num][1] then buttons[num][1].ds:Show() end
                end)
                frame:HookScript("OnLeave", StartTime)
                frame:HookScript("OnHide", function(self)
                    local num = mainFrame.frame.lastButtonNum
                    if num and buttons[num] and buttons[num][1] then buttons[num][1].ds:Hide() end
                end)
            ...
            function BG.ShowTradeHistoryInfo(self)
                ...
                local function Set(type, num)
                    ...
                    if not v[type .. "money"] or v[type .. "money"] == 0 then
                        frame[type .. "MoneyText"]:SetText("")
                    else
                        frame[type .. "MoneyText"]:SetText(GetMoneyString(v[type .. "money"], true))
                    end
                    for i = 1, 7 do
                        local vv = v[type .. "items"] and v[type .. "items"][i]
                        local itemFrame = frame[type .. "item" .. i]
                        if vv then
                            local r, g, b = GetItemQualityColor(vv.quality or 1)
                            ...
                            itemFrame.count:SetText(vv.count and vv.count > 1 and vv.count or "")
                            itemFrame.name:SetText(vv.name or "")
    ...
    -- 纯动态注入底部 Tab 按钮（位于对账右侧、邮件记录左侧）
    if BG.Create_TabButton and BG.TradeHistoryMainFrameTabNum and not BG.TradeHistoryTabButton then
...
local function InitTradeHistoryModule()
    ...
    if BiaoGe.disabledModules and BiaoGe.disabledModules["TradeHistory"] then return end
