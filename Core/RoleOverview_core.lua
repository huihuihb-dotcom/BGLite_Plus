...
local function SafeButtonIsInRight(button)
    if button and type(button) == "table" and button.GetCenter then
        local center = button:GetCenter()
        if center then
            return center > UIParent:GetWidth() * 0.5
        end
    end
    local curX = GetCursorPosition()
    local scale = UIParent:GetEffectiveScale() or 1
    return (curX / scale) > UIParent:GetWidth() * 0.5
end

local function SafeButtonIsInTop(button)
    if button and type(button) == "table" and button.GetTop then
        local top = button:GetTop()
        if top then
            return top > UIParent:GetHeight() * 0.5
        end
    end
    local _, curY = GetCursorPosition()
    local scale = UIParent:GetEffectiveScale() or 1
    return (curY / scale) > UIParent:GetHeight() * 0.5
end

local function CheckBiaoGeAccounts(frame)
    if GetAddOnInfo and IsAddOnLoaded then
        local type = select(5, GetAddOnInfo("BiaoGeAccounts"))
        if type ~= "MISSING" and not IsAddOnLoaded("BiaoGeAccounts") then
...
function BG.RoleOverviewShowAllServer()
    ...
    local defaultAll = true
    if BiaoGe and BiaoGe.options and BiaoGe.options.roleOverviewShowAllServer == 0 then
        defaultAll = false
    end
    if defaultAll then
...
function BG.SortRoleOverview(newTbl)
    ...
    sort(newTbl, function(a, b)
        ...
        if not isCustom then
            local s = (BiaoGe and BiaoGe.options and BiaoGe.options["roleOverviewSort1"]) or "level-iLevel-class-player"
...
local function FormatTitanRealmName(realmName)
    ...
    if realmName == "" then return "" end
    if BG.IsTitan then
        realmName = realmName:gsub("Ⅲ", "III"):gsub("Ⅱ", "II"):gsub("Ⅰ", "I")
            :gsub("Ⅳ", "IV"):gsub("Ⅴ", "V"):gsub("Ⅵ", "VI"):gsub("Ⅶ", "VII"):gsub("Ⅷ", "VIII")
        local base = realmName:match("^(时光[IVXivx]+)") or realmName:match("^(时光%d+)")
        if base then
            return base
        end
        realmName = realmName:gsub("%s*[%-－—–]%s*[^%-－—–]+$", ""):gsub("%s+", "")
    end
    return realmName
end
...
local function AddUseTips(t)
    ...
    local defaultShow = (BiaoGe and BiaoGe.options and BiaoGe.options.roleOverviewDefaultShow) or "all"
    local tipsText
    if defaultShow == "one" then
...
function BG.SetFBCD(self, position, click, refresh)
    ...
    BG.FBCDall_table = BG.FBCDall_table or {}
    BG.MONEYall_table = BG.MONEYall_table or {}
    ...
    local accountNames
    local accountFilter
    if click and IsAddOnLoaded and IsAddOnLoaded("BiaoGeAccounts") then
    ...
    for i, v in ipairs(BG.FBCDall_table) do
        for choicefbname, yes in pairs(BiaoGe.FBCDchoice) do
            if v.name == choicefbname and (yes == 1 or yes == true) then
    ...
    do
        ...
        if click then
            ...
            bt:SetScript("OnClick", function(self)
                ...
                if BG.ButtonOptions_roleOverview and BG.ButtonOptions_roleOverview.Click then
    ...
    if not isNewUI then
        if FBCDchoice_table and FBCDchoice_table[1] then
            FBCDchoice_table[1].name = format(L["%d个"], #DB) .. FBCDchoice_table[1].name
        end
        if MONEYchoice_table and MONEYchoice_table[1] then
    ...
    do
        ...
        if BiaoGe[FBCD] and BiaoGe[FBCD][realmID] then
            for p, v in pairs(BiaoGe[FBCD][realmID]) do
                if type(BiaoGe[FBCD][realmID][p]) == "table" then
                    for i, cd in pairs(BiaoGe[FBCD][realmID][p]) do
                        if cd.resettime then
                            if IsSmallRaid(cd.fbId) then
                                text3 = format(L["小团本%s"], SafeFormatResetTime(cd.resettime))
                            elseif cd.num ~= 5 then
                                text7 = SafeFormatResetTime(cd.resettime)
                            end
                        end
                    end
                end
    ...
    do
        ...
        if next(DB2) then
            ...
            if isNewUI and not next(DB) then
                local t_empty = mainFrame:CreateFontString()
                t_empty:SetFont(BIAOGE_TEXT_FONT, fontsize, "OUTLINE")
                t_empty:SetPoint("LEFT", leftOffset + isNewUI_TitleWidth + 15, -6 - height * 2.5)
                t_empty:SetText(BG.STC_dis(L["暂无已记录角色数据 (登录角色后自动记录)"]))
                allWidth = max(allWidth, leftOffset + isNewUI_TitleWidth + 280)
            end
        end
    end
    if isNewUI then
